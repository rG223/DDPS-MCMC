...
class PHNHyper(nn.Module):
    ...
    def __init__(self, kernel_size: List[int], ray_hidden_dim=100, out_dim=1,
                 target_hidden_dim=50, n_kernels=64, n_conv_layers=2, n_hidden=1, n_tasks=2):
        super().__init__()
        ...
        self.conv_0_weights = nn.Linear(ray_hidden_dim, n_kernels * 3 * kernel_size[0] * kernel_size[0])
        ...
        latent = 25
        self.hidden_0_weights = nn.Linear(ray_hidden_dim, target_hidden_dim * 21632)
...
class PHNTarget(nn.Module):
    """Target network
    """
    def __init__(self, kernel_size, n_kernels=64, out_dim=1, target_hidden_dim=50, n_conv_layers=2, n_tasks=2):
        super().__init__()
        assert len(kernel_size) == n_conv_layers, "kernel_size is list with same dim as number of " \
                                                  "conv layers holding kernel size for each conv layer"
    ...
    def forward(self, x, weights=None):
        x = F.conv2d(
            x, weight=weights['conv0.weights'].reshape(self.n_kernels, 3, self.kernel_size[0],
                                                       self.kernel_size[0]),
