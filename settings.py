import random
# datsets override methods override generic

#
# datasets
#
adult = dict(
    dataset='adult',
    dim=(88,),
    objectives=['BinaryCrossEntropyLoss', 'deo'],
    epochs=50,
    use_scheduler=False,
    lamda=.01,
    alpha=.5,
)

credit = dict(
    dataset='credit',
    dim=(90,),
    objectives=['BinaryCrossEntropyLoss', 'deo'],
    epochs=50,
    use_scheduler=False,
    lamda=.01,
    alpha=[.1, .5],
)

compass = dict(
    dataset='compass',
    dim=(20,),
    objectives=['BinaryCrossEntropyLoss', 'deo'],
    epochs=50,
    use_scheduler=False,
    lamda=.01,
    alpha=.5,
)

multi_mnist = dict(
    dataset='multi_mnist',
    dim=(1, 36, 36),
    objectives=['CrossEntropyLoss', 'CrossEntropyLoss'],
    lamda=8,
    alpha=1,
)

multi_fashion = dict(
    dataset='multi_fashion',
    dim=(1, 36, 36),
    objectives=['CrossEntropyLoss', 'CrossEntropyLoss'],
    lamda=2,
    alpha=1.2,
)

multi_fashion_mnist = dict(
    dataset='multi_fashion_mnist',
    dim=(1, 36, 36),
    objectives=['CrossEntropyLoss', 'CrossEntropyLoss'],
    lamda=8,
    alpha=1.2,
)

celeba = dict(
    dataset='celeba',
    dim=(3, 64, 64),
    # task_ids=[16, 22],                                    # easy tasks
    # task_ids = [2, 4],                                    # easy task and hard task
    task_ids = [25, 17],
    # task_ids = [15, 25],                                    # easy task and hard task
    # task_ids = [25, 4],
    # task_ids=[25, 27],                                    # hard tasks
    # task_ids=[2, 7, 15],                                # 3 objectives
    # task_ids=[26, 22, 24, 26],                            # 4 objectives
    # task_ids=[random.randint(0, 39) for _ in range(10)],  # 10 random tasks
    objectives=['BinaryCrossEntropyLoss' for _ in range(10)],
    n_test_rays=15,
    epochs=15,
    use_scheduler=False,
    train_eval_every=0,     # do it in parallel manually
    eval_every=1,           #
    model_name='resnet18',   # we also experimented with 'resnet-18', try it.
    lr=0.0005,
    lamda=3,
    alpha=1,
    checkpoint_every=1,
    batch_size=128,
)

#
# methods
#

paretoMTL = dict(
    method='ParetoMTL',
    num_starts=5,
    scheduler_gamma=0.5,
    scheduler_milestones=[15,30,45,60,75,90],
)

cosmos = dict(
    method='cosmos',
    lamda=2,        # Default for multi-mnist
    alpha=1.2,      #
)

balpre = dict(
    method='balpre',
    lamda=2,        # Default for multi-mnist
    alpha=1.2,      #
    PSO = False,
)

mgda = dict(
    method='mgda',
    lr=1e-4,
    approximate_norm_solution=True,
    normalization_type='loss+',
    use_scheduler=False,
)

SingleTaskSolver = dict(
    method='SingleTask',
    num_starts=2,   # two times for two objectives (sequentially)
)

uniform_scaling = dict(
    method='uniform',
)

hyperSolver_ln = dict(
    method='hyper_ln',
    lr=1e-4,
    epochs=150,
    alpha=1,   # dirichlet sampling
    use_scheduler=False,
    internal_solver='linear', # 'epo' or 'linear'
)

balhyperSolver_ln = dict(
    method='balhyper_ln',
    lr=1e-4,
    epochs=150,
    alpha=1,   # dirichlet sampling
    use_scheduler=False,
    internal_solver='linear', # 'epo' or 'linear'
    PSO = False,
)

hyperSolver_epo = dict(
    method='hyper_epo',
    lr=1e-4,
    epochs=150,
    alpha=1,   # dirichlet sampling
    use_scheduler=False,
    internal_solver='epo',

)

balhyperSolver_epo = dict(
    method='balhyper_epo',
    lr=1e-4,
    epochs=150,
    alpha=1,   # dirichlet sampling
    use_scheduler=False,
    internal_solver='epo',
    PSO = False,
)
#
# Common settings
#
generic = dict(    
    # Seed.
    seed=1,
    
    # Directory for logging the results
    logdir='results',

    # dataloader worker threads
    num_workers=4,

    # Number of test preference vectors for Pareto front generating methods    
    n_test_rays=15,

    # Evaluation period for val and test sets (0 for no evaluation)
    eval_every=5,

    # Evaluation period for train set (0 for no evaluation)
    train_eval_every=0,

    # Checkpoint period (0 for no checkpoints)
    checkpoint_every=10,

    # Use a multi-step learning rate scheduler with defined gamma and milestones
    use_scheduler=True,
    scheduler_gamma=0.1,
    scheduler_milestones=[2, 6, 10, 15],

    # Number of train rays for methods that follow a training preference (ParetoMTL and MGDA)
    num_starts=1,

    # Training parameters
    lr=1e-3,
    batch_size=256,
    epochs=100,

    # Reference point for hyper-volume calculation
    reference_point=[2, 2],
)
