...
def evaluate(j, e, solver, scores1, scores2, data_loader, logdir, reference_point, split, result_dict):
    ...
    if mode == 'pf':
        ...
        # dir_rays =np.random.dirichlet((1, 4), 15)
        # test_rays = np.concatenate([test_rays, dir_rays], axis=0)
    elif mode == 'mcr':
    ...
    for k, batch in enumerate(data_loader):
        ...
        s1 = []
        s2 = []
        if settings['method']=='balpre':
            output, output_t, test_rays = solver.eval_step(batch=batch, GP=None, split=split)
        else:
            output, test_rays = solver.eval_step(batch=batch)
        for l in output:
            batch.update(l)
...
CHECKPOINT_DIR = '/root/autodl-tmp/cosmos/results/cosmos/celeba/2023-03-03_21-41-20cosmos[2, 4]/checkpoints'
...
def eval(settings):
    ...
    for j in task_ids:
        if settings['method'] == 'SingleTask':
            ...
            checkpoints = pathlib.Path(CHECKPOINT_DIR).glob('c_0-005.pth')
